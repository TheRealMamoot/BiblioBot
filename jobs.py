...
def reserve_job():
    ...
    data: pd.DataFrame = data.sort_values(['priority','temp_datetime','temp_duration_int','temp_start'], ascending=[True, True, False, True])
    ...
    for _, row in data.iterrows():
        ...
        if (old_status=='fail' or old_status=='pending') and new_status in ['success','terminated']:
            status_change = True
...
def run_notify_job(application: Application, function):
    for hour in range(7, 23):
        for minute in [2,3,5,15,33,35,45]:
