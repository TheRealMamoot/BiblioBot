...
def schedule_slot_snapshot_job() -> None:
    ...
    trigger = CronTrigger(
        second='*/45', minute='5,10,15,25,35,36,38,40,42,45,55,57,59', hour=f'{start}-{end}', day_of_week='mon-fri'
    )
    scheduler.add_job(execute_slot_snapshot, trigger)

    trigger = CronTrigger(
        second='*/25', minute='11,13,14,17,19,20,21,22,23,27,29,50', hour=f'{start}', day_of_week='mon-fri'
    )
    scheduler.add_job(execute_slot_snapshot, trigger)
